...
namespace tmp {
    ...
    bool extractFTAResult(HFTA* hfta, FTAResult& res, int aux_size, int aux_num) {
        ...
        for (auto& aux_list: aux_plans) {
            ...
            for (auto* aux: aux_list) std::cout << "  " << aux->toString(); std::cout << std::endl;
    ...
    FTAExample buildExample(Example* example, Task* task) {
        DataList list_inputs, int_inputs = task->execute(task->target, *example);
        for (auto& v: (*example)) {
            if (v.getType() == TINT) {
                int_inputs.push_back(v);
            } else list_inputs.push_back(v);
        }
        Data oup = task->executeExample(*example).second;
    ...
    struct WorkSpace {
        ...
        bool getResult(FTAResult& res) {
            LOG(INFO) << "found " << aux_limit << " " << comb_limit;
            for (auto* fta: root->aux_fta_list) assert(fta->g == aux);

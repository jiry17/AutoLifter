...
namespace {
    ...
    MergeInfo buildMergeResult(Task* task, const std::vector<int>& id_list, int target_id, Program* res) {
        ...
        int int_num = 0, list_num = 0;
        for (auto* c: task->task_type->condition) {
            if (c->base == ConstraintInfo::NONE) int_num++;
        }
        for (auto *c: task->task_type->condition) {
        ...
        for (int id: id_list) {
            for (int j = 0; j < list_num; ++j) name_list.push_back(name_map[TLIST][j] + std::to_string(id));
        }
...
void HFTACompleteSolver::synthesis() {

...
int main(){
    ...
    for (int i = 1; i <= n; ++i) {
        x = a[i];
        res.p2 = res.p1 + x < res.p2 ? res.p2 : res.p1 + x;
        res.p1 = res.p1 + x < res.p1 + x ? res.p1 + x : res.p1 + x;
    }
    timeval end;
...
// mpsextV -- Mono clock: 2.004509s
// mpsextV -- CPU clock:  0.055928s
// Just def f(a = [a1, ..., an]):
//   p1 = array()
//   p2 = array()
//   p1[0] = CI 0
//   p2[0] = CI 0
//   for i from 1 to n:
//     p1[i] = max((id(p1[i - 1]) + id(a[i])), (id(p1[i - 1]) + id(a[i])))
//     p2[i] = max((id(p1[i - 1]) + id(a[i])), id(p2[i - 1]))

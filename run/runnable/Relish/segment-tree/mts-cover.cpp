...
void apply(int pos, int a) {
    result res = info[pos];
    tag[pos] = a;
    info[pos] = (result){((a)<=(0))?(a):((a)*(res.x2)),(res.x2)*(a),(res.x2)+(0)};
}
...
void init(int pos, int l, int r) {
    tag[pos] = 1000000000;
    if (l == r) {
        int a = w[l]; info[pos] = (result){a,a,1};
        return;
    }
    int mid = l + r >> 1;
    init(pos * 2, l, mid);
    ...
    info[pos] = (result){((rres.x1)<=((rres.x0)-(lres.x0)))?(rres.x0):((rres.x1)+(lres.x0)),(lres.x1)+(rres.x1),(rres.x2)+(lres.x2)};
}
...
void query(int pos, int l, int r, int ul, int ur) {
    ...
    if (l >= ul && r <= ur) {
        if (is_empty) {
            ...
            query_res = (result){((rres.x1)<=((rres.x0)-(lres.x0)))?(rres.x0):((rres.x1)+(lres.x0)),(lres.x1)+(rres.x1),(rres.x2)+(lres.x2)};
        }
        return;
    }
    int mid = l + r >> 1; pushdown(pos);

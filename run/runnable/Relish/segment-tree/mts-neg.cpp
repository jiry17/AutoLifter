...
void apply(int pos, int a) {
    result res = info[pos];
    tag[pos] = tag[pos]?0:1;
    info[pos] = (result){(0)-(res.x2),(0)-(res.x1),(0)-(res.x0)};
}
...
void init(int pos, int l, int r) {
    tag[pos] = 0;
    if (l == r) {
        int a = w[l]; info[pos] = (result){a,a,a};
        return;
    }
    int mid = l + r >> 1;
    init(pos * 2, l, mid);
    ...
    info[pos] = (result){((lres.x0)<=((rres.x0)-(rres.x1)))?(rres.x0):((lres.x0)+(rres.x1)),(lres.x1)+(rres.x1),((rres.x2)<=((rres.x1)+(lres.x2)))?(rres.x2):((rres.x1)+(lres.x2))};
}
...
void query(int pos, int l, int r, int ul, int ur) {
    ...
    if (l >= ul && r <= ur) {
        if (is_empty) {
            ...
            query_res = (result){((lres.x0)<=((rres.x0)-(rres.x1)))?(rres.x0):((lres.x0)+(rres.x1)),(lres.x1)+(rres.x1),((rres.x2)<=((rres.x1)+(lres.x2)))?(rres.x2):((rres.x1)+(lres.x2))};
        }
        return;
    }
    int mid = l + r >> 1; pushdown(pos);

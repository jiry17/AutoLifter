...
void apply(int pos, int a) {
    result res = info[pos];
    tag[pos] = tag[pos] + a;
    info[pos] = (result){((a)*((((res.x2)*(a))+(res.x1))+(res.x1)))+(res.x0),((res.x2)*(a))+(res.x1),(res.x2)+(0)};
}
...
void init(int pos, int l, int r) {
    ...
    info[pos] = (result){(rres.x0)+(lres.x0),(rres.x1)+(lres.x1),(rres.x2)+(lres.x2)};
}
...
void query(int pos, int l, int r, int ul, int ur) {
    ...
    if (l >= ul && r <= ur) {
        if (is_empty) {
            ...
            query_res = (result){(rres.x0)+(lres.x0),(rres.x1)+(lres.x1),(rres.x2)+(lres.x2)};
        }
        return;
    }
    int mid = l + r >> 1; pushdown(pos);

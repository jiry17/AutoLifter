...
result mergeRes(int a, const result& lres, const result& rres, int lsize, int rsize) {
    if (lsize) {
        if (rsize) {
            return (result){(((rres.x0)<(lres.x0))&&(((lres.x1)+(2))<(lres.x0)))?(lres.x0):((((lres.x1)+(2))<=(rres.x0))?(rres.x0):((lres.x1)+(2))),((rres.x3)<(lres.x3))?(rres.x1):(((rres.x2)+(lres.x1))+(1)),((lres.x2)+(rres.x2))+(1),((lres.x3)<=(rres.x3))?(lres.x3):(rres.x3)};
        } else {
            return (result){(((lres.x1)+(2))<=(lres.x0))?(lres.x0):((lres.x1)+(2)),(lres.x1)+(1),(lres.x2)+(1),lres.x3};
        }
    } else if (rsize) {
        return (result){rres.x0,rres.x1,(rres.x2)+(1),rres.x3};
    } else {
        return (result){1,0,1,a};
    }
}
...
int lsp() {
    int num = 0;
    for (int i = 1; i <= n; ++i) {
        result rres = (result){0,1000000000,0,1000000000};

...
result mergeRes(int a, const result& lres, const result& rres, int lsize, int rsize) {
    if (lsize) {
        if (rsize) {
            return (result){((((rres.x0)<(lres.x0))&&((((lres.x1)+(rres.x1))+(1))<=((a)+(lres.x2))))&&((((lres.x1)+(rres.x1))+(1))<=((a)+(rres.x2))))?(lres.x0):((((((lres.x1)+(rres.x1))+(1))<=((a)+(lres.x2)))&&((((lres.x1)+(rres.x1))+(1))<=((a)+(rres.x2))))?(rres.x0):(((lres.x1)+(rres.x1))+(1))),((lres.x1)+(rres.x1))+(1),((lres.x2)<=(rres.x2))?(lres.x2):(rres.x2)};
        } else {

...
void apply(int pos, int a) {
    result res = info[pos];
    tag[pos] = tag[pos]?0:1;
    info[pos] = (result){res.x3,(res.x5)*(-1),(res.x6)*(-1),res.x0,(res.x4)*(-1),(res.x1)*(-1),(res.x2)*(-1)};
}
...
void init(int pos, int l, int r) {
    tag[pos] = 0;
    if (l == r) {
        int a = w[l]; info[pos] = (result){((0)<=(a))?(a):(0),a,a,((0)<=(a))?(0):((a)*(-1)),a,a,a};
        return;
    }
    int mid = l + r >> 1;
    init(pos * 2, l, mid);
    ...
    info[pos] = (result){(((lres.x0)<=(rres.x0))&&(((rres.x1)+(lres.x2))<(rres.x0)))?(rres.x0):((((rres.x1)+(lres.x2))<=(lres.x0))?(lres.x0):((rres.x1)+(lres.x2))),((lres.x1)<=((lres.x4)+(rres.x1)))?((lres.x4)+(rres.x1)):(lres.x1),((rres.x2)<=((rres.x4)+(lres.x2)))?((rres.x4)+(lres.x2)):(rres.x2),(((lres.x3)<=(rres.x3))&&((((rres.x5)*(-1))+((lres.x6)*(-1)))<=(rres.x3)))?(rres.x3):(((((rres.x5)*(-1))+((lres.x6)*(-1)))<=(lres.x3))?(lres.x3):(((rres.x5)*(-1))+((lres.x6)*(-1)))),(lres.x4)+(rres.x4),(((lres.x4)+(rres.x5))<=(lres.x5))?((lres.x4)+(rres.x5)):(lres.x5),(((rres.x4)+(lres.x6))<=(rres.x6))?((rres.x4)+(lres.x6)):(rres.x6)};
}
...
void query(int pos, int l, int r, int ul, int ur) {
    ...
    if (l >= ul && r <= ur) {
        if (is_empty) {
            ...
            query_res = (result){(((lres.x0)<=(rres.x0))&&(((rres.x1)+(lres.x2))<(rres.x0)))?(rres.x0):((((rres.x1)+(lres.x2))<=(lres.x0))?(lres.x0):((rres.x1)+(lres.x2))),((lres.x1)<=((lres.x4)+(rres.x1)))?((lres.x4)+(rres.x1)):(lres.x1),((rres.x2)<=((rres.x4)+(lres.x2)))?((rres.x4)+(lres.x2)):(rres.x2),(((lres.x3)<=(rres.x3))&&((((rres.x5)*(-1))+((lres.x6)*(-1)))<=(rres.x3)))?(rres.x3):(((((rres.x5)*(-1))+((lres.x6)*(-1)))<=(lres.x3))?(lres.x3):(((rres.x5)*(-1))+((lres.x6)*(-1)))),(lres.x4)+(rres.x4),(((lres.x4)+(rres.x5))<=(lres.x5))?((lres.x4)+(rres.x5)):(lres.x5),(((rres.x4)+(lres.x6))<=(rres.x6))?((rres.x4)+(lres.x6)):(rres.x6)};
        }
        return;
    }
    int mid = l + r >> 1; pushdown(pos);

...
void apply(int pos, int a) {
    result res = info[pos];
    tag[pos] = tag[pos] + a;
    info[pos] = (result){((res.x0)+(((a)*(res.x2))*(2)))+((a)*((a)*(res.x1))),res.x1,(res.x2)+((a)*(res.x1))};
}
...
void init(int pos, int l, int r) {
    tag[pos] = 0;
    if (l == r) {
        int a = w[l]; info[pos] = (result){(a)*(a),1,a};
        return;
    }
    int mid = l + r >> 1;
    init(pos * 2, l, mid);
    ...
    info[pos] = (result){(lres.x0)+(rres.x0),(lres.x1)+(rres.x1),(lres.x2)+(rres.x2)};
}
...
void query(int pos, int l, int r, int ul, int ur) {
    ...
    if (l >= ul && r <= ur) {
        if (is_empty) {
            ...
            query_res = (result){(lres.x0)+(rres.x0),(lres.x1)+(rres.x1),(lres.x2)+(rres.x2)};
        }
        return;
    }
    int mid = l + r >> 1; pushdown(pos);

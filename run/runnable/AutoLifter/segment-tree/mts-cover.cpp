...
void apply(int pos, int a) {
    result res = info[pos];
    tag[pos] = a;
    info[pos] = (result){((a)<=(0))?(a):((a)*(res.x2)),(a)*(res.x2),res.x2};
}
...
void init(int pos, int l, int r) {
    ...
    info[pos] = (result){((rres.x0)<=((rres.x1)+(lres.x0)))?((rres.x1)+(lres.x0)):(rres.x0),(lres.x1)+(rres.x1),(lres.x2)+(rres.x2)};
}
...
void query(int pos, int l, int r, int ul, int ur) {
    ...
    if (l >= ul && r <= ur) {
        if (is_empty) {
            ...
            query_res = (result){((rres.x0)<=((rres.x1)+(lres.x0)))?((rres.x1)+(lres.x0)):(rres.x0),(lres.x1)+(rres.x1),(lres.x2)+(rres.x2)};
        }
        return;
    }
    int mid = l + r >> 1; pushdown(pos);

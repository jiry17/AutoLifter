...
void apply(int pos, int a) {
    result res = info[pos];
    tag[pos] = a;
    info[pos] = (result){((a)<=(0))?(0):((a)*(res.x3)),((a)<=(0))?(a):((a)*(res.x3)),((a)<=(0))?(a):((a)*(res.x3)),res.x3,(a)*(res.x3)};
}
void pushdown(int pos) {
    if (tag[pos] == 1000000000) return;
    apply(pos * 2, tag[pos]);
...
void init(int pos, int l, int r) {
    tag[pos] = 1000000000;
    if (l == r) {
        int a = w[l]; info[pos] = (result){((a)<=(0))?(0):(a),a,a,1,a};
        return;
    }
    int mid = l + r >> 1;
    init(pos * 2, l, mid);
    ...
    info[pos] = (result){(((lres.x0)<(rres.x0))&&(((rres.x1)+(lres.x2))<=(rres.x0)))?(rres.x0):((((rres.x1)+(lres.x2))<=(lres.x0))?(lres.x0):((rres.x1)+(lres.x2))),((lres.x1)<=((lres.x4)+(rres.x1)))?((lres.x4)+(rres.x1)):(lres.x1),((rres.x2)<=((rres.x4)+(lres.x2)))?((rres.x4)+(lres.x2)):(rres.x2),(lres.x3)+(rres.x3),(lres.x4)+(rres.x4)};
}
...
void query(int pos, int l, int r, int ul, int ur) {
    ...
    if (l >= ul && r <= ur) {
        if (is_empty) {
            ...
            query_res = (result){(((lres.x0)<(rres.x0))&&(((rres.x1)+(lres.x2))<=(rres.x0)))?(rres.x0):((((rres.x1)+(lres.x2))<=(lres.x0))?(lres.x0):((rres.x1)+(lres.x2))),((lres.x1)<=((lres.x4)+(rres.x1)))?((lres.x4)+(rres.x1)):(lres.x1),((rres.x2)<=((rres.x4)+(lres.x2)))?((rres.x4)+(lres.x2)):(rres.x2),(lres.x3)+(rres.x3),(lres.x4)+(rres.x4)};
        }
        return;
    }
    int mid = l + r >> 1; pushdown(pos);

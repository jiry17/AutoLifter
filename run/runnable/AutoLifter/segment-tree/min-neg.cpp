...
void init(int pos, int l, int r) {
    ...
    info[pos] = (result){((lres.x0)<=(rres.x0))?(lres.x0):(rres.x0),((rres.x1)<=(lres.x1))?(lres.x1):(rres.x1)};
}
...
void query(int pos, int l, int r, int ul, int ur) {
    ...
    if (l >= ul && r <= ur) {
        if (is_empty) {
            ...
            query_res = (result){((lres.x0)<=(rres.x0))?(lres.x0):(rres.x0),((rres.x1)<=(lres.x1))?(lres.x1):(rres.x1)};
        }
        return;
    }
    int mid = l + r >> 1; pushdown(pos);

...
#pragma omp task shared(rres) if(r - l > lim)
        rres = dac(mid + 1, r);
#pragma omp taskwait
        return (result){(((rres.x1)<(0))&&((rres.x0)<=((lres.x2)*(rres.x2))))?((lres.x2)*(rres.x2)):((((rres.x0)<=(rres.x1))&&((rres.x0)<=((lres.x0)*(rres.x0))))?((lres.x0)*(rres.x0)):(rres.x0)),(lres.x1)*(rres.x1),((((rres.x1)*(lres.x2))<=(rres.x2))&&((1)<=(rres.x1)))?((rres.x1)*(lres.x2)):(((((lres.x0)*(rres.x2))<=(rres.x2))&&((rres.x1)<=(rres.x2)))?((lres.x0)*(rres.x2)):(rres.x2))};
    }
}
int main(){
    scanf("%d",&n);

...
result dac(int l, int r)
{
    if (l == r) {
        int a = w[l];
        return (result){((a)<=(1))?(0):(a),((a)<=(1))?(0):(a),((a)<=(1))?(0):(a),a};
    } else {
        result lres, rres;
        int mid = l + r >> 1;
...
#pragma omp task shared(rres) if(r - l > lim)
        rres = dac(mid + 1, r);
#pragma omp taskwait
        return (result){(((rres.x0)<=(lres.x0))&&(((rres.x1)+(lres.x2))<(lres.x0)))?(lres.x0):((((rres.x1)+(lres.x2))<=(rres.x0))?(rres.x0):((rres.x1)+(lres.x2))),((lres.x3)<=(lres.x1))?((lres.x1)+(rres.x1)):(lres.x1),((rres.x3)<=(rres.x2))?((lres.x2)+(rres.x2)):(rres.x2),(lres.x3)+(rres.x3)};
    }
}
int main(){
    scanf("%d",&n);

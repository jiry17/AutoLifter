...
#pragma omp task shared(rres) if(r - l > lim)
        rres = dac(mid + 1, r);
#pragma omp taskwait
        return (result){(((lres.x0)<=(rres.x0))&&(((rres.x1)+(lres.x2))<=(rres.x0)))?(rres.x0):((((rres.x1)+(lres.x2))<=(lres.x0))?(lres.x0):((rres.x1)+(lres.x2))),((lres.x1)<=((lres.x3)+(rres.x1)))?((lres.x3)+(rres.x1)):(lres.x1),((rres.x2)<=((rres.x3)+(lres.x2)))?((rres.x3)+(lres.x2)):(rres.x2),(lres.x3)+(rres.x3)};
    }
}
int main(){
    scanf("%d",&n);

...
result mergeRes(int a, const result& lres, const result& rres, int lsize, int rsize) {
    if (lsize) {
        if (rsize) {
            return (result){(((((lres.x1)+(rres.x1))+(1))<=(a))&&((rres.x0)<=(lres.x0)))?(lres.x0):(((((lres.x1)+(rres.x1))+(1))<=(a))?(rres.x0):(((lres.x1)+(rres.x1))+(1))),((lres.x1)+(rres.x1))+(1)};
        } else {
            return (result){((a)<=(lres.x1))?((lres.x1)+(1)):(lres.x0),(lres.x1)+(1)};
        }
    } else if (rsize) {
        return (result){(((rres.x0)+(1))<=(a))?(rres.x0):((rres.x0)+(1)),(rres.x1)+(1)};
    } else {
        return (result){((1)<=(a))?(0):(1),1};
    }
}

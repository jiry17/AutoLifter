...
result dac(int l, int r)
{
    if (l == r) {
        int a = w[l];
        return (result){0,((a)<=(0))?(1000000000):(1),a};
    } else {
        result lres, rres;
        int mid = l + r >> 1;
...
#pragma omp task shared(rres) if(r - l > lim)
        rres = dac(mid + 1, r);
#pragma omp taskwait
        return (result){(((((rres.x1)!=(1000000000))&&((lres.x0)<(rres.x1)))&&((rres.x0)<=(0)))||((((lres.x1)==(1000000000))&&((lres.x0)<=(0)))&&((rres.x0)<=(0))))?(0):(1),((rres.x1)!=(1000000000))?(lres.x2):(rres.x1),lres.x2};
    }
}
int main(){
    scanf("%d",&n);

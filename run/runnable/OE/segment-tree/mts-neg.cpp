...
void apply(int pos, int a) {
    result res = info[pos];
    tag[pos] = tag[pos]?0:1;
    info[pos] = (result){(res.x2)*(-1),(res.x1)*(-1),(res.x0)*(-1)};
}
...
void init(int pos, int l, int r) {
    ...
    info[pos] = (result){((rres.x0)<=((rres.x1)+(lres.x0)))?((rres.x1)+(lres.x0)):(rres.x0),(lres.x1)+(rres.x1),(((rres.x1)+(lres.x2))<=(rres.x2))?((rres.x1)+(lres.x2)):(rres.x2)};
}
...
void query(int pos, int l, int r, int ul, int ur) {
    ...
    if (l >= ul && r <= ur) {
        if (is_empty) {
            ...
            query_res = (result){((rres.x0)<=((rres.x1)+(lres.x0)))?((rres.x1)+(lres.x0)):(rres.x0),(lres.x1)+(rres.x1),(((rres.x1)+(lres.x2))<=(rres.x2))?((rres.x1)+(lres.x2)):(rres.x2)};
        }
        return;
    }
    int mid = l + r >> 1; pushdown(pos);

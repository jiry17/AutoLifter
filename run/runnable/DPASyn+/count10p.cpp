...
int main(){
    ...
    for (int i = 1; i <= n; ++i) {
        x = a[i];
        res.p2 = (0 == x) ? (0 + res.p1 + res.p2) : res.p2;
        res.p1 = (1 == 0) ? 1 : (0 + x + 0);
    }
    timeval end;
...
//   p2[0] = CI 0
//   for i from 1 to n:
//     p1[i] = ite((one(p2[i - 1]) == zero(p2[i - 1])), one(p1[i - 1]), ((zero(p2[i - 1]) + id(a[i])) + zero(p2[i - 1])))
//     p2[i] = ite((zero(p2[i - 1]) == id(a[i])), ((zero(p2[i - 1]) + id(p1[i - 1])) + id(p2[i - 1])), id(p2[i - 1]))
//   return p2[n]
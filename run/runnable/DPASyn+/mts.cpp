...
int main(){
    ...
    for (int i = 1; i <= n; ++i) {
        x = a[i];
        res.p2 = (x + res.p1 < 0 + 0) ? 0 + 0 : x + res.p1;
        res.p1 = (x + res.p1 < 0 + 0) ? 0 + 0 : x + res.p1;
    }
    timeval end;
...
//   p2[0] = CI 0
//   for i from 1 to n:
//     p1[i] = max((id(a[i]) + id(p1[i - 1])), (zero(p1[i - 1]) + zero(p1[i - 1])))
//     p2[i] = max((id(a[i]) + id(p1[i - 1])), (zero(p1[i - 1]) + zero(p1[i - 1])))
